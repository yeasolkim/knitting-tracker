...
export default function SubPatternSelector({
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="truncate max-w-[120px]">{active?.name || '도안 1'}</span>
        <svg
      ...
      {/* Dropdown */}
      {isExpanded && (
        <div className="absolute bottom-full left-0 mb-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1 min-w-[240px] z-50">
          {subPatterns.map((sub) => (
            <div
              key={sub.id}
              className={`px-3 py-2 hover:bg-gray-50 ${sub.id === activeId ? 'bg-rose-50' : ''}`}
            >
              {editingId === sub.id ? (
                <input
                  autoFocus
