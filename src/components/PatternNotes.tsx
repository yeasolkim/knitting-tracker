    ...
    <div>
      ...
      {isOpen && (
        <div className="mt-1.5 space-y-2">
          {/* Current row input */}
          <div className="bg-[#f5edd6] border-2 border-[#c4a07a] rounded-lg p-2.5">
            <div className="flex items-center gap-1.5 mb-1.5">
              <span className="text-[10px] font-bold tracking-widest uppercase text-[#b5541e] bg-[#fdf6e8] border border-[#c4a07a] px-1.5 py-0.5 rounded">
            ...
            <textarea
              ...
              className="w-full h-16 text-sm border-2 border-[#c4a07a] rounded-lg p-2 resize-none focus:outline-none focus:border-[#b5541e] bg-[#fdf6e8] text-[#3d2b1f] placeholder:text-[#c4a882]"
            />
            <div className="flex items-center justify-end gap-1.5 mt-1.5">
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-lg border-2 transition-all ${
                  hasUnsaved
                    ...
                    : 'bg-[#fdf6e8] text-[#a08060] border-[#c4a07a] opacity-60'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1 px-2 py-1.5 text-xs font-bold rounded-lg border-2 transition-colors ${
                  hasUnsaved
                    ? 'bg-[#fdf6e8] text-[#c4a882] border-[#c4a07a] opacity-40 cursor-not-allowed'
          ...
          {groupedNotes.length > 0 && (
            <div ref={scrollRef} className="max-h-28 sm:max-h-48 overflow-y-auto border-t-2 border-[#c4a07a] pt-2 space-y-3">
                    ...
                    <span
                      className={`text-[10px] font-bold tracking-wide px-1.5 py-0.5 rounded border ${
                        sub.id === activeSubPattern.id
                          ...
                          : 'text-[#7a5c46] bg-[#f5edd6] border-[#c4a07a]'
                      }`}
                    >
                      {sub.name}
                    </span>
                          ...
                          {isEditing ? (
                            <div className="flex-1 flex flex-col gap-1">
                              <textarea
                                ...
                                className="w-full h-14 text-sm border-2 border-[#c4a07a] rounded-lg p-1.5 resize-none focus:outline-none focus:border-[#b5541e] bg-[#fdf6e8] text-[#3d2b1f]"
                              />
                          ...
                          {!isEditing && (
                            <div className="flex items-center shrink-0 -mr-1.5">
                              <button
                                ...
                                className={`p-1.5 min-h-[44px] min-w-[44px] flex items-center justify-center transition-colors ${
                                  notePositions[key]
                                    ...
                                    : 'text-[#c4a07a] hover:text-amber-400 sm:opacity-0 sm:group-hover:opacity-100'
                                }`}
                                title={t('notes.placeNote')}
                              >
                              ...
                              <button
                                ...
                                className="text-[#c4a07a] hover:text-[#b5541e] sm:opacity-0 sm:group-hover:opacity-100 transition-all p-1.5 min-h-[44px] min-w-[44px] flex items-center justify-center"
                                aria-label="메모 삭제"
                              >
                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
