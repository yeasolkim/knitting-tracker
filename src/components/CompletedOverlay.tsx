...
const CompletedOverlay = memo(function CompletedOverlay({ marks, onUpdate, onDelete, onDeleteAll, onSelectionChange, onDragStart }: CompletedOverlayProps) {
  ...
  const dragStartRef = useRef<{ clientY: number; startY: number; startH: number; clientX: number; startX: number } | null>(null);
  ...
  const handlePointerDown = useCallback(
    (index: number, mode: DragMode) => (e: React.PointerEvent) => {
      ...
      dragStartRef.current = {
        ...
        clientX: e.clientX,
        startX: marks[index].y, // y field stores position for both orientations
      };
    },
    [marks, onDragStart]
  );
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const isHorizontal = mark.orientation === 'horizontal';
      ...
      if (isHorizontal) {
        ...
        if (dragMode === 'move') {
          onUpdate(selectedIndex, { ...mark, y: dragStartRef.current.startX + delta });
        } else if (dragMode === 'resize-top') {
          const newY = dragStartRef.current.startX + delta;
          const newH = dragStartRef.current.startH - delta;
          if (newH >= 0.5) onUpdate(selectedIndex, { ...mark, y: newY, height: newH });
        } else if (dragMode === 'resize-bottom') {
          const newH = dragStartRef.current.startH + delta;
          if (newH >= 0.5) onUpdate(selectedIndex, { ...mark, height: newH });
        }
      } else {
        ...
        if (dragMode === 'move') {
          onUpdate(selectedIndex, { ...mark, y: dragStartRef.current.startY + delta });
        } else if (dragMode === 'resize-top') {
    ...
    [dragMode, selectedIndex, marks, toPercentY, toPercentX, onUpdate]
  );
            ...
            >
              ...
              <div
                className={`w-full h-full transition-colors cursor-pointer ${
                  isSelected
                    ? 'bg-emerald-400/30 border-x-2 border-emerald-500/70'
                    : 'bg-emerald-400/15 border-x border-emerald-500/40'
                }`}
                onPointerDown={isSelected ? handlePointerDown(i, 'move') : undefined}
                style={{ cursor: isSelected ? 'grab' : 'pointer' }}
              >
                <div className="absolute bottom-1 left-1/2 -translate-x-1/2 text-emerald-600/60 text-[10px] font-medium">
                  ✓
                </div>
              </div>
              ...
              {isSelected && (
                <>
                  {/* Left resize handle */}
                  <div
                    className="absolute top-0 bottom-0 left-0 w-3 cursor-ew-resize z-20 flex items-center justify-center -translate-x-1/2"
                  ...
                  {/* Right resize handle */}
                  <div
                    className="absolute top-0 bottom-0 right-0 w-3 cursor-ew-resize z-20 flex items-center justify-center translate-x-1/2"
                  ...
                  {/* Delete button */}
                  <div className="absolute top-1 left-1/2 -translate-x-1/2 flex gap-1 z-30">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onDelete(i);
