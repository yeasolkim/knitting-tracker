...
const RowRuler = memo(function RowRuler({
  ...
  return (
    ...
    >
      ...
      {/* Fine position nudge buttons — centered on top/bottom edges, right side */}
      <div
        className="absolute right-3 pointer-events-auto z-20"
        style={{ top: `${positionY}%`, transform: 'translateY(-50%)' }}
      >
        <button
          onPointerDown={(e) => e.stopPropagation()}
          onClick={(e) => { e.stopPropagation(); onChangePosition(Math.max(0, positionY - 0.3)); }}
          className="flex items-center justify-center w-8 h-8 rounded-full bg-[#fdf6e8]/90 border border-[#d4b896] text-[#b07840] shadow-md hover:bg-[#f5edd6] hover:text-[#b5541e] active:scale-95 transition-all"
        >
      ...
      <div
        className="absolute right-3 pointer-events-auto z-20"
        style={{ top: `${positionY + height}%`, transform: 'translateY(-50%)' }}
      >
        <button
          onPointerDown={(e) => e.stopPropagation()}
