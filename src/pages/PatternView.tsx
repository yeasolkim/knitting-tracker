...
function PatternViewerPage({ pattern }: Props) {
  ...
  const [rulerHeight, setRulerHeight] = useState(Math.min(pattern.progress?.ruler_height || 0.3, 1.8));
  ...
  const handleRulerHeightChange = useCallback((screenHPct: number) => {
    ...
    const refH = iH > 0 ? iH : H;
    setRulerHeight(Math.min(1.8, Math.max(0.001, (contentH_px / refH) * 100)));
  }, []);
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              <input
                ...
                max={10000}
                step={1}
                value={Math.round(rulerHeight / 1.8 * 10000)}
                onPointerDown={captureHistory}
                onChange={(e) => {
                  setIsAdjustingRuler(true);
                  setRulerHeight(Math.max(0.001, Number(e.target.value) / 10000 * 1.8));
                }}
