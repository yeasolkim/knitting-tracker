...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (isValidLang(metaLang)) {
        ...
        localStorage.setItem('knitting_in_the_sauna_lang', metaLang);
      }
    });
  ...
  const setLang = useCallback((newLang: Lang) => {
    ...
    localStorage.setItem('knitting_in_the_sauna_lang', newLang);
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) {
...
export function LanguageToggle() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-36 bg-[#fdf6e8] border-2 border-[#b07840] rounded-xl shadow-[3px_3px_0_#b07840] z-50 overflow-hidden">
          {LANGUAGES.map((l) => (
            <button
              key={l.code}
